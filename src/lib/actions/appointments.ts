...
export async function updateAppointmentStatus(input: { id: string; status: AppointmentStatus }) {
  try {
    ...
    // find appointment to check ownership
    ...
    const isOwner = existingAppointment.user.clerkId === user?.id;

    if (!isAdmin && !isOwner) {
      throw new Error("Unauthorized: You don't have permission to update this appointment.");
    }
